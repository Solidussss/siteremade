...
function createPurchaseIntent(db, { ownerId, projectId, amount, currency = 'cad' }) {
  const project = db.prepare('SELECT * FROM projects WHERE id = ? AND owner_id = ?').get(projectId, ownerId);
  ...
  db.prepare(`INSERT INTO purchase_intents (id, owner_id, project_id, status, amount, currency, created_at, updated_at)
              VALUES (?, ?, ?, 'pending', ?, ?, ?, ?)`)
    .run(id, ownerId, projectId, amount, currency, ts, ts);
  db.prepare(`UPDATE projects SET status = 'checkout_pending', updated_at = ? WHERE id = ? AND status = 'draft'`).run(ts, projectId);
  return { ok: true, intent: { id, projectId, status: 'pending' } };
}
function attachStripeSession(db, intentId, stripeSessionId) {
  db.prepare('UPDATE purchase_intents SET stripe_session_id = ?, updated_at = ? WHERE id = ?').run(stripeSessionId, nowIso(), intentId);
}
...
function getOwnedPurchaseIntent(db, ownerId, intentId) {
  const row = db.prepare('SELECT * FROM purchase_intents WHERE id = ? AND owner_id = ?').get(intentId, ownerId);
  if (!row) return null;
...
function fulfillBySessionId(db, stripeSessionId) {
  const intent = db.prepare('SELECT * FROM purchase_intents WHERE stripe_session_id = ?').get(stripeSessionId);
  ...
  try {
    db.prepare(`UPDATE purchase_intents SET status = 'fulfilled', updated_at = ? WHERE id = ? AND status = 'pending'`).run(ts, intent.id);
    db.prepare(`UPDATE projects SET status = 'purchased', purchase_ref = ?, updated_at = ? WHERE id = ?`).run(stripeSessionId, ts, intent.project_id);
    db.exec('COMMIT');
...
function markIntentTerminal(db, stripeSessionId, status) {
  ...
  db.exec('BEGIN IMMEDIATE');
  try {
    db.prepare('UPDATE purchase_intents SET status = ?, updated_at = ? WHERE id = ?').run(status, ts, intent.id);
    db.prepare(`UPDATE projects SET status = 'draft', updated_at = ? WHERE id = ? AND status = 'checkout_pending'`).run(ts, intent.project_id);
    db.exec('COMMIT');
  } catch (e) { db.exec('ROLLBACK'); throw e; }
