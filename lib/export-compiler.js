...
// Product-flow pass: owner-facing handoff docs/resources (see buildHandoff
// below) -- kept separate from render.js/runtime-classifier.js, which are
// about the SITE itself, not about the paperwork that ships alongside it.
const { listHandoffResources } = require('./handoff-resources.js');
...
function renderDocument({ direction, category, page, pageIndex, pages, businessName, routes }) {
  ...
  const dataAttrs = [
    ...
    // Responsive/QA pass: matches render.renderHero's OWN effective layout
    // (see lib/site-render.js's effectiveHeroLayout) -- previously this
    // read the raw, never-downgraded `dims.hero`, so an unfunded hero's
    // data-hero attribute silently disagreed with the actual rendered
    // markup below it.
    `data-hero="${render.escapeHtml(render.effectiveHeroLayout(direction) || 'split')}"`,
    ...
    // DESIGN INTELLIGENCE PASS: icon/decorative direction, same
    // archetype-derived fields script.js's applyDesignDataset stamps on
    // the live preview's .builder-site -- keeps the CSS attribute
    // selectors that key off these (styles.css) working identically in a
    // purchased export.
    `data-icon-weight="${render.escapeHtml(dims.iconWeight || 'regular')}"`,
    `data-icon-density="${render.escapeHtml(dims.iconDensity || 'medium')}"`,
    `data-icon-presentation="${render.escapeHtml(dims.iconPresentation || 'icon-led-row')}"`,
    `data-decorative-motif="${render.escapeHtml(dims.decorativeMotif || 'none')}"`,
    // FINAL BASELINE POLISH pass, part 5: archetype-derived motion
    // character (restrained/direct/tactile/structured/typographic/soft),
    // same archetypeIconDefaults-table mechanism as the icon/decorative
    // fields above -- keeps styles.css's `[data-motion-character]` scaling
    // rules working identically in a purchased export.
    `data-motion-character="${render.escapeHtml(dims.motionCharacter || 'direct')}"`,
  ].join(' ');
  // PREMIUM_GENERATION_V1: same token system the live preview applies (sanitised; only --site-* vars with safe values).
  const premiumTokens = require('./premium').tokens.sanitizeTokens(direction.design && direction.design.premiumTokens);
  const premiumStyle = premiumTokens ? Object.keys(premiumTokens.vars).map(k => `${k}:${premiumTokens.vars[k]}`).join(';') : '';
  const premiumAttrs = premiumTokens ? ` data-premium="1" data-premium-type="${render.escapeHtml(premiumTokens.typographyKey)}"${premiumTokens.composition ? ' data-comp="v2"' : ''}` : '';
...
<body>
<div class="builder-site" id="builderSite" ${dataAttrs}${premiumAttrs} style="${styleAttr}${premiumStyle ? ';' + premiumStyle : ''}">
...
(function () {
  ...
  // FINAL BASELINE POLISH pass, part 4/6 (micro-motion system): ONE
  // IntersectionObserver total (never one per element) drives a one-shot
  // fade+rise reveal on every top-level .site-hero/.site-page-header/
  // .site-section the first time it scrolls into view on a real visitor's
  // page -- it never re-triggers on scroll back up ("no excessive/
  // distracting motion"). Process steps additionally stagger in
  // individually via pure CSS (site.css's .sr-revealed .process-flow>*
  // nth-child(N) rules) the moment their own section reveals, so no second
  // observer is needed for that. This is the real, scroll-triggered version
  // of script.js's initSiteMotion -- see that function's own comment for
  // why the in-editor preview deliberately runs a simpler, non-scroll-
  // triggered copy instead (a re-render-per-keystroke editor has no
  // equivalent to a real page's one-time load).
  //
  // Respects TWO independent "off" signals, never conflated:
  // prefers-reduced-motion (a person's own OS-level accessibility
  // preference, always honored -- part 6) and the pre-existing per-category
  // data-motion="none" dimension already set on #builderSite by the
  // export compiler (some categories, e.g. finance/roofing, already choose
  // motion:'none' on their own). Either one skips the observer entirely and
  // leaves every section in its final, fully visible state immediately --
  // no flash-then-fade, no motion library, no scroll-jacking, no
  // requestAnimationFrame loop, nothing beyond this one observer and CSS
  // transitions (part 6's explicit constraints).
  var motionRoot = document.getElementById('builderSite');
  var prefersReducedMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  var motionDisabled = (motionRoot && motionRoot.getAttribute('data-motion') === 'none') || prefersReducedMotion;
  var revealTargets = Array.prototype.slice.call(document.querySelectorAll('.site-hero, .site-page-header, .site-section'));
  if (revealTargets.length) {
    if (motionDisabled || !window.IntersectionObserver) {
      revealTargets.forEach(function (el) { el.classList.add('sr-revealed'); });
    } else {
      revealTargets.forEach(function (el) { el.classList.add('sr-reveal'); });
      var pendingReveal = revealTargets.slice();
      ...
      function revealOne(el) {
        ...
        if (!pendingReveal.length) window.removeEventListener('scroll', scrollCatchUp);
      }
      // A single very fast, instant jump (keyboard End, a hash-link
      // landing, a fling-scroll) can move the viewport past a section in
      // one rendering step with no intermediate frame for the observer to
      // ever sample -- the crossing itself is skipped, not just its
      // threshold, and a one-shot reveal would leave that section stuck at
      // opacity:0 forever. This is a lightweight safety net for exactly
      // that case, not a second animation driver: ONE shared, passive
      // scroll listener (never per-element, never a requestAnimationFrame
      // loop, never altering scroll position) that only reads bounding
      // rects for whatever is still pending and reveals anything already
      // on-screen or fully scrolled past -- content a visitor scrolled
      // straight through doesn't need to animate in, it just must not stay
      // invisible. It detaches itself the moment nothing is left pending.
      function scrollCatchUp() {
        pendingReveal.slice().forEach(function (el) {
          var r = el.getBoundingClientRect();
          if (r.bottom <= 0 || r.top <= window.innerHeight) revealOne(el);
        });
      }
...
// Phase 6: an optional, opt-in "Visitor analytics" section for BOTH runtime
// types. Nothing is injected into the exported pages -- the owner (or their
// developer) adds one script tag by hand if they want SiteRemade analytics.
// The tag names this export's builder project id, never a domain or an
// account/workspace key: the SiteRemade app resolves which business it
// belongs to on its own side (its GET /api/public/analytics-config-by-project,
// through the same workspace <-> project link Contact submissions use), so
// changing the site's domain later never breaks it.
function buildAnalyticsSection(projectId) {
  const pid = typeof projectId === 'string' && /^proj_[A-Za-z0-9_-]{8,64}$/.test(projectId) ? projectId : '<this export\'s projectId from export-manifest.json>';
  return `## Visitor analytics (optional)\n\nIf you use the SiteRemade app, it can show visits to this site in its Analytics view. Nothing is added automatically -- if you want it, paste this one line just before \`</body>\` in every page you want counted (index.html and each other .html page in this folder):\n\n\`\`\`html\n<script defer src="https://<your SiteRemade app address>/siteremade-analytics.js?project=${pid}"></script>\n\`\`\`\n\nIt identifies this site by its SiteRemade project id (\`${pid}\`, also in export-manifest.json), not by its domain -- moving the site to a different domain later needs no change here. No account key or password goes in your pages; the SiteRemade app works out which business the site belongs to on its own side. Visits are only counted once this website is linked to your business in the SiteRemade app (that happens automatically the first time you open the Website view there after purchase, signed in with the same SiteRemade account you bought it with) and its analytics are set up -- until then the tag quietly does nothing, and it never changes how your site looks or works. Remove the line to stop.\n\n`;
}
function buildReadme({ runtimeType, businessName, projectId }) {
  ...
  if (runtimeType === 'static') {
    return common + `## Hosting\n\nThis is a static site: index.html, any additional page files, styles.css, site.js, and an assets/ folder. Upload the contents of this folder to any static host (see export-manifest.json for the technical requirements a hosting recommendation is based on).\n\n` + buildAnalyticsSection(projectId) + `Before going live, update sitemap.xml's placeholder domain to your real one.\n`;
  }
  return common + `## Hosting\n\nThis site needs a live Node.js runtime (it has real, functioning contact/quote/booking/newsletter form(s) -- see export-manifest.json's runtimeReasons). It has no npm dependencies.\n\n### Run it\n\n\`\`\`\nnode server.js\n\`\`\`\n\nIt listens on the PORT environment variable, or 8080.\n\n### Form submissions\n\nBy default (SUBMISSION_BACKEND=local, or unset), submissions are validated server-side and saved to data/submissions.jsonl only -- nothing is sent anywhere else.\n\nTo actually deliver submissions to your own email/CRM/automation system, set:\n\n- \`SUBMISSION_BACKEND=webhook\`\n- \`SUBMISSION_WEBHOOK_URL\` -- the HTTPS endpoint that will receive each validated submission as a JSON POST (fields, form type, and this export's projectId/artifactHash/revision)\n- \`SUBMISSION_WEBHOOK_SECRET\` (optional) -- if set, each request is signed with an \`X-SiteRemade-Signature\` HMAC-SHA256 header so your endpoint can verify it came from this server\n\nIf SUBMISSION_BACKEND=webhook is set without SUBMISSION_WEBHOOK_URL, the server refuses to start rather than silently falling back to local-only storage. A submission is only reported as successful to your visitor once the configured backend actually confirms it (a real local file write, or a real 2xx response from your webhook) -- a failed delivery is reported as a real failure, never hidden.\n\n### Sending submissions to your SiteRemade Contact inbox (optional)\n\nIf you use the SiteRemade app, you can point the webhook at it: set \`SUBMISSION_BACKEND=webhook\` and \`SUBMISSION_WEBHOOK_URL=https://<your SiteRemade app address>/api/public/site-submission\`. Each submission already carries this export's projectId, and the SiteRemade app matches that to your business on its own side -- nothing else to configure, and no account key goes in this package. (\`SUBMISSION_WEBHOOK_SECRET\` isn't needed for this and the SiteRemade app doesn't check it.) It only works once your website is linked to your business in the SiteRemade app -- that happens automatically the first time you open the Website view there after purchase, signed in with the same SiteRemade account you bought it with; before that the app refuses the submission and your visitor sees a real error, so send one test submission after setting it up. Changing your site's domain later doesn't affect it.\n\n` + buildAnalyticsSection(projectId) + `Before going live, update sitemap.xml's placeholder domain to your real one.\n`;
}

// ---- HANDOFF.md (product-flow pass) ----------------------------------------
// Owner-facing (README.md above stays developer-facing) -- what you bought,
// how to make simple changes yourself, domain/hosting basics in plain
// language, and where to go for more help. Deliberately does NOT explain
// how to edit code (that's README.md's job) and does NOT fabricate any
// host-specific click-by-click UI steps for a provider this codebase
// hasn't actually implemented a real handoff for (see lib/hosting.js --
// every provider besides 'local' is honestly `available:false` here).
function buildHandoff({ businessName, runtimeType, hostingChoice, purchaseDate }) {
  const resources = listHandoffResources();
  const resourceLines = resources.map(r => `- **${r.title}** — ${r.url ? r.url : '_not yet available -- check back, or see the support resources below_'}`).join('\n');
  const purchaseLine = purchaseDate ? `Purchased on ${purchaseDate}.\n\n` : '';
  const hostingSection = (() => {
    if (!hostingChoice || hostingChoice.skipped || !hostingChoice.provider || hostingChoice.provider === 'self') {
      return `You chose to host this yourself (or haven't picked a host yet). See README.md's "Hosting" section for the technical requirements, and \`export-manifest.json\` for what this specific site needs.`;
    }
    return `You chose **${hostingChoice.provider}** as your hosting provider during checkout. This is a recommendation only -- your ownership of this website does not depend on it, and you're free to host anywhere. Sign up with your chosen provider, then follow their own upload/deploy instructions using the files in this package (see README.md for the technical shape of what you're uploading).`;
  })();
  return `# ${businessName} — your website, explained\n\n${purchaseLine}This document is for YOU, the site owner -- not a developer. For technical setup/run instructions, see README.md in this same folder.\n\n## What you bought\n\nA complete, standalone copy of your website's source files. It does not depend on SiteRemade, your SiteRemade account, or any SiteRemade server to keep running -- these files are yours to host, edit, and hand to any developer, forever.\n\n## Making simple changes\n\nThis exported copy is plain HTML/CSS/JS, not a SiteRemade project -- editing it directly means editing those files by hand (or handing them to a developer). If you'd rather keep editing visually and export a fresh copy afterward, go back into SiteRemade, make your changes there, and download a new export from "My Websites" in your account -- this package will not update itself.\n\n- **Text**: open the relevant \`.html\` file in any text editor and change the words between tags.\n- **Images**: replace the matching file under \`assets/\` with a new image of the same name (or update the \`src\`/\`href\` reference if you rename it).\n- **Contact info**: search each \`.html\` file for your phone number/email/address and update every occurrence.\n\n## Publishing changes\n\nAfter editing, re-upload the changed files to wherever you're hosting this site (see the Hosting section below). There's no separate "publish" step beyond putting the updated files where your host serves them from.\n\n## Hosting\n\n${hostingSection}\n\n## Your domain\n\nPointing a domain you already own at this site is a DNS change made with whoever you bought the domain from (or your hosting provider, if they also sell domains) -- see README.md for the technical basics. We never claim one-click domain setup where it doesn't exist yet.\n\n## Support & resources\n\n${resourceLines}\n\nFor anything else, reach out through your SiteRemade account.\n`;
}
...
function compileExport(db, { project, directionIndex, workDir, hostingChoice, purchaseDate }) {
  ...
  const manifest = {
    ...
    // Product-flow pass: purely descriptive of what's in this package's own
    // HANDOFF.md -- never re-interpreted by anything that loads this file.
    hostingChoice: hostingChoice || null,
    handoffResources: listHandoffResources(),
  };
  fs.writeFileSync(path.join(workDir, 'export-manifest.json'), JSON.stringify(manifest, null, 2));
  fs.writeFileSync(path.join(workDir, 'README.md'), buildReadme({ runtimeType, businessName, projectId: project.id }));
  fs.writeFileSync(path.join(workDir, 'HANDOFF.md'), buildHandoff({ businessName, runtimeType, hostingChoice: hostingChoice || null, purchaseDate: purchaseDate || null }));
