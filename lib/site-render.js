...
// object-position from a stored focal point ("72% 55%"); anything else is ignored.
function focalStyle(focal) { return (typeof focal === 'string' && /^\d{1,3}% \d{1,3}%$/.test(focal)) ? ` style="object-position:${focal}"` : ''; }
function renderVisualSlot(project, slot, imageryKey, assetId) {
  ...
  if (asset && asset.dataUrl) return `<img class="site-visual-img" src="${escapeHtml(asset.dataUrl)}"${focalStyle(asset.focal)} alt="${escapeHtml(asset.alt || (project.business.name || 'Business') + ' image')}" />`;
  // A resolved GENERATED image (hydrated by the export compiler to a relative asset path) must render as an image. Before
  // PREMIUM_GENERATION_V1 this branch did not exist here, so a purchased export always showed the placeholder for generated
  // slots (paid images were off by default, so it was never noticed).
  const generatedEntry = project.assets && project.assets.generated && project.assets.generated[slot];
  if (generatedEntry && generatedEntry.status === 'ready' && generatedEntry.dataUrl) return `<img class="site-visual-img site-visual-generated-img" src="${escapeHtml(generatedEntry.dataUrl)}"${focalStyle(generatedEntry.focal)} alt="${escapeHtml((project.business.name || 'Business') + ' image')}" />`;
...
function isVisualSlotFunded(project, slot, assetId) {
  ...
  const planEntry = (project.imagePlan || []).find(p => p.slot === slot);
  const gen = project.assets && project.assets.generated && project.assets.generated[slot];
  if (gen && gen.status === 'ready' && gen.dataUrl) return true;
...
function renderCtaBanner(project, category, section, submitMode) {
  ...
  const message = sectionCopyField(section, 'headline', compositionActive(project)
    ? premiumLib.composition.finalCtaHeadline(premiumLib.stamp.strategyFor(project), { location: project.source && project.source.location, noun: category.noun, businessName: project.business && project.business.name })
...
// PREMIUM_COMPOSITION_V2 (export parity with script.js): data-driven from the project's stored tokens.
function compositionActive(project) {
  try { const t = project && project.design && premiumLib.tokens.sanitizeTokens(project.design.premiumTokens); return !!(t && t.composition); } catch (e) { return false; }
}
...
function renderPageBody(project, page, isHome, category, submitMode) {
  ...
  let contentHtml, footerHtml = renderSiteFooter(project, category, project.footerVariant || 'simple');
  if (compositionActive(project)) {
    // PREMIUM_COMPOSITION_V2: same plan the live preview computes, stamped onto each section's opening tag.
    const parts = (page.sections || []).filter(s => s && s.type !== 'footer').map(s => ({ section: s, html: renderSectionHTML(project, s, category, submitMode) }));
    const stamped = premiumLib.stamp.stampParts(project, parts, { variant: effectiveHeroLayout(project), hasImage: /<img[^>]*site-visual-img/.test(introHtml) });
    contentHtml = stamped.html.join('');
    footerHtml = premiumLib.stamp.stampFooterHtml(footerHtml, stamped.plan);
  } else {
    contentHtml = (page.sections || []).map(s => renderSectionHTML(project, s, category, submitMode)).join('');
  }
