...
// ---- Row <-> API shape ----------------------------------------------------
// Phase 9: `source` is provenance-only ({type:'new'|'redesign', url,
// importedAt, metadata}) -- see migrations/0007_project_source.sql. Always
// present (type defaults to 'new' at the DB layer), so every existing
// caller of rowToSummary/rowToFull keeps working unchanged and just gets
// one more, always-populated field.
function rowToSource(row) {
  let metadata = null;
  if (row.source_metadata_json) {
    try { metadata = JSON.parse(row.source_metadata_json); } catch (e) { metadata = null; }
  }
  return {
    type: row.source_type === 'redesign' ? 'redesign' : 'new',
    url: row.source_url || null,
    importedAt: row.source_imported_at || null,
    metadata,
  };
}
function rowToSummary(row) {
  return { id: row.id, name: row.name, status: row.status, purchaseRef: row.purchase_ref || null, revision: row.revision, createdAt: row.created_at, updatedAt: row.updated_at, source: rowToSource(row) };
}
...
// Phase 9: optional 4th field `source` records provenance for a project
// created from an existing-site redesign (see lib/site-import.js). Omitted
// entirely (every pre-Phase-9 call site) -> source_type stays the DB
// default 'new', byte-identical to prior behavior. `source.metadata` is
// reference material collected by the import/extraction step -- capped and
// string-only, same discipline as every other client-influenced payload in
// this file, never trusted to contain executable content.
function validateSourceInput(source) {
  if (!source || typeof source !== 'object') return null;
  if (source.type !== 'redesign') return null; // only 'redesign' is ever explicitly recorded; 'new' is the implicit default
  if (!validateHttpUrl(source.url)) return null; // no url, no redesign provenance -- refuse rather than record a half-formed record
  return {
    sourceType: 'redesign',
    sourceUrl: String(source.url).slice(0, 2000),
    sourceImportedAt: source.importedAt || nowIso(),
    sourceMetadataJson: source.metadata && typeof source.metadata === 'object'
      ? JSON.stringify(capStringsDeep(source.metadata, { maxLen: 4000, maxArray: 60, maxKeys: 60, depth: 5 }))
      : null,
  };
}
function createProject(db, ownerId, { name, directionsState, sourceLocalId, source } = {}) {
  if (sourceLocalId) {
  ...
  const id = genId('proj');
  const ts = nowIso();
  const sourceFields = validateSourceInput(source) || {};
  db.projects.insert({
...
module.exports = {
  ...
  // Phase 9: exported so lib/site-import.js / the redesign extraction route
  // can validate a `source` object with the exact same rules before ever
  // constructing the payload passed to createProject, and so tests can
  // assert on the shape directly.
  rowToSource, validateSourceInput,
};
