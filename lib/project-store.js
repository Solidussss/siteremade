...
const ASSET_STORE_DIR = path.join(__dirname, '..', 'data', 'asset-store');
...
function validateDirection(raw) {
  ...
  return {
    ...
    design: raw.design && typeof raw.design === 'object' ? capStringsDeep(raw.design, { maxLen: 200, maxArray: 20, maxKeys: 20, depth: 4 }) : {},
...
function ensureAssetStoreDir() { fs.mkdirSync(ASSET_STORE_DIR, { recursive: true }); }
function internalizeOneEntry(db, entry) {
  ...
  const filePath = path.join(ASSET_STORE_DIR, hash);
  if (!fs.existsSync(filePath)) fs.writeFileSync(filePath, buf);
  const existing = db.prepare('SELECT hash FROM asset_blobs WHERE hash = ?').get(hash);
  if (!existing) db.prepare('INSERT INTO asset_blobs (hash, content_type, byte_length, created_at) VALUES (?, ?, ?, ?)').run(hash, m[1], buf.length, nowIso());
...
function hydrateOneEntry(db, entry) {
  ...
  const filePath = path.join(ASSET_STORE_DIR, entry.assetRef);
  let buf;
  try { buf = fs.readFileSync(filePath); } catch (e) { return entry; }
...
function listOwnedProjects(db, ownerId) {
  return db.prepare('SELECT * FROM projects WHERE owner_id = ? ORDER BY updated_at DESC').all(ownerId).map(rowToSummary);
}
function getOwnedProject(db, ownerId, projectId) {
  const row = db.prepare('SELECT * FROM projects WHERE id = ? AND owner_id = ?').get(projectId, ownerId);
...
function getOwnedProjectStatus(db, ownerId, projectId) {
  const row = db.prepare('SELECT id, status, purchase_ref, updated_at FROM projects WHERE id = ? AND owner_id = ?').get(projectId, ownerId);
...
function createProject(db, ownerId, { name, directionsState, sourceLocalId } = {}) {
  if (sourceLocalId) {
    const existing = db.prepare('SELECT * FROM projects WHERE owner_id = ? AND source_local_id = ?').get(ownerId, String(sourceLocalId).slice(0, 120));
  ...
  db.prepare(`INSERT INTO projects (id, owner_id, name, status, source_local_id, state_json, revision, created_at, updated_at)
              VALUES (?, ?, ?, 'draft', ?, ?, 1, ?, ?)`)
    .run(id, ownerId, String(name || 'Untitled project').slice(0, 200), sourceLocalId ? String(sourceLocalId).slice(0, 120) : null, JSON.stringify(check.normalized), ts, ts);
...
function updateOwnedProject(db, ownerId, projectId, { name, directionsState, expectedRevision } = {}) {
  ...
  // The revision check is repeated in the WHERE clause (compare-and-swap)
  // as real defense in depth, even though this whole function already runs
  ...
  const result = db.prepare(`UPDATE projects SET name = ?, state_json = ?, revision = revision + 1, updated_at = ?
    ...
    .run(newName, JSON.stringify(normalized), ts, projectId, ownerId, row.revision);
  if (result.changes === 0) {
    const fresh = db.prepare('SELECT * FROM projects WHERE id = ? AND owner_id = ?').get(projectId, ownerId);
...
function archiveProject(db, ownerId, projectId) {
  ...
  db.prepare(`UPDATE projects SET status = 'archived', updated_at = ? WHERE id = ?`).run(nowIso(), projectId);
  return { ok: true };
}
...
module.exports = {
  ...
  listOwnedProjects, getOwnedProject, getOwnedProjectRaw, getOwnedProjectStatus, createProject, updateOwnedProject, archiveProject,
