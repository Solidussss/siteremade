...
function createAccount(db, email, password) {
  ...
  const existing = db.prepare('SELECT id FROM accounts WHERE email = ?').get(check.email);
  if (existing) return { ok: false, error: 'An account with this email already exists.' };
  const id = genId('acct');
  const ts = nowIso();
  db.prepare('INSERT INTO accounts (id, email, password_hash, created_at, updated_at) VALUES (?, ?, ?, ?, ?)')
    .run(id, check.email, hashPassword(password), ts, ts);
  return { ok: true, account: { id, email: check.email } };
}
function findAccountByEmail(db, email) {
  return db.prepare('SELECT * FROM accounts WHERE email = ?').get(normalizeEmail(email)) || null;
}
...
function createSession(db, accountId) {
  ...
  db.prepare('INSERT INTO sessions (id, account_id, created_at, expires_at) VALUES (?, ?, ?, ?)')
    .run(hashToken(token), accountId, ts, expiresAt);
...
function resolveSession(db, token) {
  ...
  const row = db.prepare('SELECT s.account_id as accountId, s.expires_at as expiresAt, a.email as email FROM sessions s JOIN accounts a ON a.id = s.account_id WHERE s.id = ?').get(id);
  if (!row) return null;
